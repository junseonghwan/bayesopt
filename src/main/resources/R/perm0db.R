...
perm0db <- function(xx, b=10)
{
  if (!is.matrix(xx)) {
    ...
    n<-dim(xx)[1]
    d<-dim(xx)[2]
    jj<-matrix(rep(1:d, n), ncol=d, byrow=T)
    
    # unless d is super large, this shouldn't be too slow
    inner<-matrix(0, nrow = n, ncol = d)
    for (i in 1:d)
    {
      inner[,i]<-rowSums((jj + b)*(xx^i - 1/jj^i))^2
    }
    y<-rowSums(inner)
    return(y)
  }
